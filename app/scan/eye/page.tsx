...
import { ArrowLeft, Eye, Activity } from "lucide-react"
    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8">
        <div className="text-center mb-6 anim-fade-up">
          <div className="inline-flex items-center gap-2 glass-card !rounded-full px-4 py-2 !shadow-sm text-sm font-medium text-teal-600 mb-4">
            <Eye className="w-4 h-4" />
            AI-Powered Analysis
          </div>
          <h1 className="text-3xl sm:text-4xl font-semibold text-zinc-900 tracking-tight mb-2" style={{ fontFamily: 'var(--font-outfit)' }}>{t.scanPages.eye.title}</h1>
          <p className="text-zinc-500 max-w-2xl mx-auto">{t.scanPages.eye.desc}</p>
        </div>
        <HealthChatbot scanType="eye" title={t.scanPages.eye.title} description={t.scanPages.eye.chatDesc} acceptedFiles="image/*" />
      </div>
    </div>
  )
}
